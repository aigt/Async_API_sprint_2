...
@pytest.mark.asyncio
async def test_persons(es_write_data, make_get_request, query_data, expected):
...
@pytest.mark.asyncio
async def test_persons_full_name(es_write_data, make_get_request, query_data, expected):
    bulk_query = get_es_bulk_query(data=es_persons,
                                   index=test_settings.es_index['persons'],
                                   id_field='id')
...
@pytest.mark.parametrize("person_id, expected",
                         [(es_persons[1]['id'], {'status': 200,
                                                 ...
                                                 'role': ['actor']})])
@pytest.mark.asyncio
async def test_persons_id(es_write_data, make_get_request, person_id, expected):
    bulk_query = get_es_bulk_query(data=es_persons,
                                   index=test_settings.es_index['persons'],
                                   id_field='id')

    await es_write_data(bulk_query=bulk_query, index=test_settings.es_index['persons'])
    url = test_settings.service_url + '/api/v1/persons/' + person_id
...
@pytest.mark.asyncio
async def test_persons_invalid_id(es_write_data, make_get_request, person_id, expected):
    bulk_query = get_es_bulk_query(data=es_persons,
                                   index=test_settings.es_index['persons'],
                                   id_field='id')
...
@pytest.mark.asyncio
async def test_persons_films(es_write_data, make_get_request, person_id, expected):
    bulk_query = get_es_bulk_query(data=es_persons,
                                   index=test_settings.es_index['persons'],
                                   id_field='id')

    await es_write_data(bulk_query=bulk_query, index=test_settings.es_index['persons'])
    url = test_settings.service_url + '/api/v1/persons/' + person_id + '/film'
